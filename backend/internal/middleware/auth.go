...
// ClientAuth middleware para proteger rotas do cliente
func ClientAuth(c *fiber.Ctx) error {
	...
	isProvider, ok := claims["isProvider"].(bool)
	if !ok || isProvider {
		return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"error": "Acesso negado. Apenas clientes permitidos."})
	}

	c.Locals("clientID", claims["id"])
	return c.Next()
}
